 ...
 */
class PermissionFragment : Fragment(), PermissionCallbacks {
    ...
    override fun onPermissionsDenied(requestCode: Int, denied: List<String>) {
        if (denied.isNotEmpty()) {
            ...
            if (requestCode == EasyPermissions.PERM_REQ_CODE && isAvailable(activity)) {
                // DES: 获取拒绝的权限的名称集合
                val labelRes = activity.application.applicationInfo.labelRes
                ...
                val onClickListener =
                    DialogInterface.OnClickListener { _, _ -> detachFragment(this@PermissionFragment) }
                val askAgain = EasyPermissions.checkDeniedPermissionsNeverAskAgain(
                    this, rationale, R.string.aop_permission_tip, R.string.aop_permission_setting, R.string.aop_permission_denied,
                    onClickListener, denied)
                if (!askAgain) {
